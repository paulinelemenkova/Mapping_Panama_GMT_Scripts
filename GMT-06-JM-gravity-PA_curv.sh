...
gmt img2grd curv_27.1.img -R276.5/283/6.5/10.5 -Ggrav_v_PA.grd -T1 -I1 -E -S0.1 -V
gdalinfo grav_v_PA.grd -stats

# Generate a color palette table from grid
gmt makecpt -Canotherrainbow.cpt -T-120/150 > colors.cpt

# Generate a file
ps=Grav_PA_v.ps

gmt grdimage grav_v_PA.grd -Ccolors.cpt -R276.5/283/6.5/10.5 -JM6.0i -P -I+a15+ne0.75 -Xc -K > $ps

# Add isolines
gmt grdcontour grav_v_PA.grd -R -J -C50 -A100 -Wthinner -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Vertical gradient of gravity anomaly for Panama" -O -K >> $ps
...
gmt psscale -Dg276.5/6.05+w15.3c/0.4c+h+o0.0/0i+ml+e -R -J -Ccolors.cpt \
    ...
    -Bg20f2a20+l"Color scale 'anotherrainbow' by Peggy Jentoft (sets of gradients for digital art: Set four) [C=RGB -T-120/200]" \
...
gmt psconvert Grav_PA_v.ps -A0.5c -E720 -Tj -Z
